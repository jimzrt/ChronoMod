...
#ifdef _MSC_VER
#define SWAP_ULONG _byteswap_ulong
#else
#define SWAP_ULONG __builtin_bswap32
#endif // _MSC_VER
...
void ResourceBin::load_header()
{
    ...
    uncompressed_length_header = SWAP_ULONG(uncompressed_length_header);
...
std::vector<char> ResourceBin::extract(const ResourceEntry& entry)
{
    ...
    entry_uncompressed_length = SWAP_ULONG(entry_uncompressed_length);
...
void ResourceBin::create_with_modifications(std::unordered_map<std::string, Patch>& patchMap, const std::string& path, const std::function<void(int)>& progress_callback)
{
...
#endif // NDEBUG
            //buffer = std::move(tmp);
            entry_length = SWAP_ULONG(entry_length - 4);
    ...
    int32_t uncompressed_length_header_swapped = SWAP_ULONG(uncompressed_length_header);
